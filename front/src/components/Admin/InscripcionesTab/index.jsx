                            ...
                            return gruposAgrupados.map((grupoCurso) => (
                                <div key={grupoCurso.curso.id} style={{ marginBottom: '2rem' }}>
                                    ...
                                    {grupoCurso.dictados.map((grupoDictado) => {
                                        const isExpanded = dictadosExpandidos[grupoDictado.dictado.id] ?? true;

                                        return (
                                            <div key={grupoDictado.dictado.id} style={{ marginBottom: '1.5rem' }}>
                                                <div
                                                ...
                                                >
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                                                        ...
                                                        <div>
                                                            <strong>{grupoDictado.dictado.diasSemana.join(', ')}</strong>
                                                            {' • '}
                                                            {grupoDictado.dictado.horarioInicio} - {grupoDictado.dictado.horarioFin}
                                                        </div>
                                                    </div>
                                                    <div style={{ fontSize: '0.9rem', color: '#666' }}>
                                                        {grupoDictado.inscripciones.length} {grupoDictado.inscripciones.length === 1 ? 'inscripción' : 'inscripciones'}
                                                    </div>
                                                </div>

                                                {isExpanded && grupoDictado.inscripciones.map((ins) => (
                                                    <InscripcionCard
                                                        key={ins.id}
                                                        inscripcion={ins}
