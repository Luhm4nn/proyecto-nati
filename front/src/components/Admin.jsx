...
function Admin() {
  ...
  const getEstadoColor = (estado) => {
    switch (estado) {
      ...
      case 'contactada': return 'estado-contactada'
      default: return ''
    }
  }
    ...
    <div className="admin">
      <div className="admin-container">
        ...
        {loading ? (
          ...
          <div className="empty-state">No hay solicitudes {filtro !== 'todas' ? `en estado "${filtro}"` : ''}</div>
        ) : (
            ...
            {solicitudes.map((sol) => (
              <div key={sol.id} className="solicitud-card">
                <div className="solicitud-header">
                  <div>
                    <h3>{DOMPurify.sanitize(sol.nombre, {ALLOWED_TAGS: []})}</h3>
                    <span className={`estado-badge ${getEstadoColor(sol.estado)}`}>
                      {sol.estado}
                    </span>
                  </div>
                ...
                <div className="solicitud-body">
                  ...
                  {sol.telefono && <p><strong>Teléfono:</strong> {DOMPurify.sanitize(sol.telefono, {ALLOWED_TAGS: []})}</p>}
                  <p><strong>Mensaje:</strong> {DOMPurify.sanitize(sol.mensaje, {ALLOWED_TAGS: []})}</p>
                </div>
